...
def generate_answer_response(messages: list) -> str:
    """Non-streaming answer generation fallback."""
    tokens = list(stream_answer_response(messages))
    return "".join(tokens)

def stream_answer_response(messages: list):
    """
    Communicates with local Ollama instance using Phi-4-mini with stream=True.
